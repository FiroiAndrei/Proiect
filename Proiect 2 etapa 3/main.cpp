...
class Film
{
    ...
    */
    ...
    double rating; /// rating este un numar oarecare, cu cat mai mult cu atat mai bine-
...
public:
     ...
     Film(bool anim, char restr, char* ser,
            char categ[], float dur, double rat,string dir, char nume[]):director(dir)
     {
...
//
    ...
    this->seria = new char[strlen(f.seria)+1];
    strcpy(this->seria, f.seria);
...
public:
    ...
    void adaugare(Film &a)
    {
        lista[this->indice] = new Film();
        *lista[this->indice] = a;
        this->indice++;
    }
...
class User
{
    ...
    */
    ...
    int nrNote = 0;
    public:
    set<int> reports; // daca un user isi ia report de la 3 useri unici atunci acesta va fi banat
    list <int> cumparat; // id-ul chestiilor din Merch (daca Userul a cumparat ceva)
...
class UserFidel : public User
{
    bool donat; /// adica daca a donat sau nu
...
public:
    bool getabonat()
    {
     ...
     void grad()
    {
        if (this->donat == true && this->getNrNote() > 10 && this->getPremium() == true)
            cout << "User fidel si bun\n" << endl;
        else
            if (this->donat == true && this->getMembru() == 'A')
            cout << "VIP care a donat\n";
        else if (this->getMembru() == 'A')
            cout << "VIP\n";
        else
            cout << "Nu stiu\n";
...
class Merch
{
    char* produs;
    int pret;
    const float euro; // curs valutar in lei
     map<int, char> id; //fiecare produs are un id si o categorie
...
class ActoriBuni : public Actori
{
    int premiiOscar;
...
public:
    int getpremiiOscar()
    {
        return this->premiiOscar;
    }

    void grad()
    {
        if (this->premiiOscar == 1)
            cout << "Leonardo DiCaprio" << endl;
        else
            if (this->premiiOscar < 10)
            cout << "Artist";
        else
            cout << "Mare om";
    }
